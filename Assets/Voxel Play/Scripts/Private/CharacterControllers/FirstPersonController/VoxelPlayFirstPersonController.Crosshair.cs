    ...
    public partial class VoxelPlayFirstPersonController : VoxelPlayCharacterControllerBase
    {
        ...
        void LateUpdate ()
        {
            ...
            if (env.cameraHasMoved || forceUpdateCrosshair) {
                ...
                float hitRange = VoxelPlayPlayer.instance.GetHitRange ();
                crosshairOnBlock = env.RayCast (ray, out crosshairHitInfo, env.buildMode ? crosshairMaxDistance : hitRange, colliderTypes: ColliderTypes.IgnorePlayer) && crosshairHitInfo.voxelIndex >= 0;
                if (changeOnBlock) {
                    if (crosshairOnBlock) {
