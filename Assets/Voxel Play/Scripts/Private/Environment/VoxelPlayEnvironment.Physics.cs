...
namespace VoxelPlay
{
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour
    {
        ...
        bool RayCastFast (Vector3 origin, Vector3 direction, out VoxelHitInfo hitInfo, float maxDistance = 0, bool createChunksIfNeeded = false, byte minOpaque = 0, ColliderTypes colliderTypes = ColliderTypes.AnyCollider, int layerMask = -1)
        {
        ...
        bool HitVoxelFast (Vector3 origin, Vector3 direction, int damage, out VoxelHitInfo hitInfo, float maxDistance = 0, int damageRadius = 1, bool addParticles = true, bool playSound = true, bool allowDamageEvent = true, int layerMask = -1)
        {

            RayCastFast (origin, direction, out hitInfo, maxDistance, false, 0, ColliderTypes.IgnorePlayer, layerMask);
        ...
        int DamageAreaFast (Vector3 origin, int damage, int damageRadius = 1, bool distanceAttenuation = true, bool addParticles = true, List<VoxelIndex> results = null, bool playSound = false, bool showDamageCracks = false, bool canAddRecoverableVoxel = true)
        {
            ...
            for (int k = 0; k < count; k++) {
                ...
                if (dam > 0) {
                    ...
                    int damageTaken = DamageVoxelFast (ref hitInfo, dam, addParticles, playSound, true, showDamageCracks, canAddRecoverableVoxel);
                    if (hasResults) {
                        VoxelIndex di = vi;
        ...
        int DamageVoxelFast (ref VoxelHitInfo hitInfo, int damage, bool addParticles, bool playSound, bool allowDamageEvent = true, bool showDamageCracks = true, bool canAddRecoverableVoxel = true)
        {
            ...
            if (allowDamageEvent && OnVoxelAfterDamaged != null) {
        ...
        /// </summary>
        public virtual void ParticleBurst (Vector3 position, ParticleBurstStyle style, int particleCount, float intensity, VoxelDefinition materialVoxelDefinition = null, Gradient colors = null)
        {
        ...
        public virtual bool CheckCollision (Vector3 position)
        {
            int x, y, z;
