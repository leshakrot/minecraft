...
namespace VoxelPlay
{
    ...
    public class BiomeLookUp
    {
        ...
        public FastList<BiomeZone> overlappingZones = new FastList<BiomeZone> ();

        public BiomeLookUp (BiomeZone zone, bool uniqueZone)
        {
            this.zone = zone;
            this.uniqueZone = uniqueZone;
        }

        public BiomeDefinition GetBiome (float altitude, float moisture)
        {
            if (uniqueZone) return zone.biome;
            for (int k = 0; k < overlappingZones.count; k++) {
                if (altitude >= overlappingZones.values [k].altitudeMin && altitude < overlappingZones.values [k].altitudeMax &&
                    moisture >= overlappingZones.values [k].moistureMin && moisture < overlappingZones.values [k].moistureMax) {
                    return overlappingZones.values [k].biome;
                }
            }
            return zone.biome;
        }
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour
    {
        ...
        /// </summary>
        protected BiomeLookUp [] biomeLookUp;
        ...
        void InitBiomes ()
        {
            biomeLookUp = new BiomeLookUp [441]; // 21 * 21
            if (world == null)
                return;
            ...
            BiomeZone defaultZone = new BiomeZone ();
            if (world.defaultBiome != null) {
                world.defaultBiome.ValidateSettings ();
                if (world.defaultBiome.zones != null && world.defaultBiome.zones.Length > 0) {
                    defaultZone = world.defaultBiome.zones [0];
                }
            }

            for (int k = 0; k < biomeLookUp.Length; k++) {
                biomeLookUp [k] = new BiomeLookUp (defaultZone, true);
            }

            if (world.biomes == null)
                return;

            float maxHeight = world.terrainGenerator != null ? world.terrainGenerator.maxHeight : 255;
            for (int b = 0; b < world.biomes.Length; b++) {
                BiomeDefinition biome = world.biomes [b];
                if (biome == null)
                    continue;
                biome.ValidateSettings ();
                if (biome.zones == null) continue;
                ...
                for (int z = 0; z < biome.zones.Length; z++) {
                    ...
                    for (int elevation = 0; elevation <= 20; elevation++) {
                        ...
                        if (e0 > zone.altitudeMax || e1 < zone.altitudeMin) continue;
                        for (int moisture = 0; moisture <= 20; moisture++) {
                            ...
                            if (m0 <= zone.moistureMax && m1 >= zone.moistureMin) {
                                ...
                                if (!biomeLookUp [lookupIndex].overlappingZones.Contains (zone)) {
                                    biomeLookUp [lookupIndex].overlappingZones.Add (zone);
                                    if (biomeLookUp [lookupIndex].overlappingZones.count > 1) {
                                        biomeLookUp [lookupIndex].uniqueZone = false;
                                    }
                                }
                                biomeLookUp [lookupIndex].zone = zone;
                                if (zone.altitudeMin > e0 || zone.altitudeMax < e1 || zone.moistureMin > m0 || zone.moistureMax < m1) {
                                    biomeLookUp [lookupIndex].uniqueZone = false;
                                }
                            }
                        }
            ...
            // for all non-unique zones, add the default zone at the end
            for (int k = 0; k < biomeLookUp.Length; k++) {
                if (!biomeLookUp [k].uniqueZone && !biomeLookUp [k].overlappingZones.Contains (defaultZone)) {
                    biomeLookUp [k].overlappingZones.Add (defaultZone);
                }
            }
        ...
        public HeightMapInfo GetHeightMapInfoFast (float x, float z)
        {
            ...
            if (!heightMapCache.TryGetValue (ix, iz, out heights, out heightsIndex)) {
                ...
                heights [heightsIndex].biome = biomeLookUp [biomeIndex].GetBiome (groundLevel, moisture);
            }
        ...
        void UpdateAndNotifyChunkChanges (float endTime)
        {
            meshingIdle = true;
            if (meshingThreads != null) {
                for (int k = 0; k < meshingThreads.Length; k++) {
                    UpdateAndNotifyChunkChanges (meshingThreads [uploadingMeshingThreadIndex], endTime);
        ...
        void UpdateAndNotifyChunkChanges (MeshingThread thread, float endTime)
        {
