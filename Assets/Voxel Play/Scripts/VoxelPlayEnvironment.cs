...
namespace VoxelPlay
{
    ...
    public partial class VoxelPlayEnvironment : MonoBehaviour
    {
        ...
        /// <param name="hitInfo">VoxelHitInfo structure with additional details.</param>
        /// <param name="maxDistance">Max distance of ray.</param>
        public bool RayHit (Vector3 origin, Vector3 direction, int damage, out VoxelHitInfo hitInfo, float maxDistance = 0, int damageRadius = 1, int layerMask = -1)
        {
        ...
        public bool RayCast (Ray ray, out VoxelHitInfo hitInfo, float maxDistance = 0, int minOpaque = 0, ColliderTypes colliderTypes = ColliderTypes.AnyCollider, int layerMask = -1)
        {
        ...
        /// <param name="colliderTypes">Optionally specify which colliders can be used</param>
        /// <param name="layerMask">Optional layer mask to filter collider-based (non voxel) objects</param>
        public bool RayCast (Vector3 origin, Vector3 direction, out VoxelHitInfo hitInfo, float maxDistance = 0, int minOpaque = 0, ColliderTypes colliderTypes = ColliderTypes.AnyCollider, int layerMask = -1)
        {
        ...
        /// </summary>
        public bool IsTerrainReadyAtPosition (Vector3 position, bool includeWater)
        {
        ...
        /// <param name="chunk">Chunk.</param>
        /// <param name="forceCreation">If set to <c>true</c> force creation.</param>
        public VoxelChunk GetChunk (Vector3 position, bool forceCreation = false)
        {
            int chunkX, chunkY, chunkZ;
            FastMath.FloorToInt (position.x / CHUNK_SIZE, position.y / CHUNK_SIZE, position.z / CHUNK_SIZE, out chunkX, out chunkY, out chunkZ);
            VoxelChunk chunk;
        ...
        public bool GetVoxelIndex (VoxelChunk chunk, int voxelIndex, int offsetX, int offsetY, int offsetZ, out VoxelChunk otherChunk, out int otherVoxelIndex, bool createChunkIfNotExists = false)
        {
            ...
            Vector3 position;
            position.x = chunk.position.x - CHUNK_HALF_SIZE + 0.5f + qx;
            position.y = chunk.position.y - CHUNK_HALF_SIZE + 0.5f + qy;
            position.z = chunk.position.z - CHUNK_HALF_SIZE + 0.5f + qz;
            FastMath.FloorToInt (position.x / CHUNK_SIZE, position.y / CHUNK_SIZE, position.z / CHUNK_SIZE, out chunkX, out chunkY, out chunkZ);
        ...
        /// </summary>
        public void GetVoxelNeighbourhood (Vector3 position, ref VoxelIndex [] voxelIndices)
        {
            VoxelChunk chunk;
            int voxelIndex;
            if (GetVoxelIndex (position, out chunk, out voxelIndex)) {
        ...
        public int VoxelDamage (Vector3 origin, int damage, int radius, bool attenuateDamageWithDistance, bool addParticles, bool playSound = false, bool showDamageCracks = false, bool canAddRecoverableVoxel = true)
        {
        ...
        /// <param name="canAddRecoverableVoxel">If true, when voxel is destroyed, a floating recoverable voxel can be dropped.</param>
        public int VoxelDamage (Vector3 origin, int damage, int radius, bool attenuateDamageWithDistance, bool addParticles, List<VoxelIndex> damagedVoxels, bool playSound = false, bool canAddRecoverableVoxel = true)
        {
        ...
        /// </summary>
        public int GetVoxelResistancePoints (Vector3 position)
        {
            VoxelChunk chunk;
            int voxelIndex;
            if (GetVoxelIndex (position, out chunk, out voxelIndex)) {
        ...
        public BiomeDefinition GetBiome (float altitude, float moisture)
        {
            ...
            if (biomeIndex >= 0 && biomeIndex < biomeLookUp.Length) {
                int groundLevel = world.terrainGenerator != null ? (int)(altitude * world.terrainGenerator.maxHeight) : (int)(altitude * 255);
                return biomeLookUp [biomeIndex].GetBiome (groundLevel, moisture);
            }
            return null;
        }
        ...
        public float GetVoxelLight (Vector3 position, out VoxelChunk chunk, out int voxelIndex)
        {
            ...
            if (GetVoxelIndex (position, out chunk, out voxelIndex, false) && chunk.lightmapSignature >= 0) {
        ...
        /// </summary>
        /// <returns>The light intensity.</returns>
        public int GetVoxelLightPacked (Vector3 position)
        {
            int voxelIndex;
            VoxelChunk chunk;
            return GetVoxelLightPacked (position, out chunk, out voxelIndex);
        }
        ...
        /// </summary>
        /// <returns>The light intensity at the voxel position..</returns>
        public int GetVoxelLightPacked (Vector3 position, out VoxelChunk chunk, out int voxelIndex)
        {
            chunk = null;
            voxelIndex = 0;
            if (!effectiveGlobalIllumination) {
            ...
            if (GetVoxelIndex (position, out chunk, out voxelIndex, false) && chunk.lightmapSignature >= 0) {
                if (chunk.voxels [voxelIndex].lightMeshOrTorch != 0 || chunk.voxels [voxelIndex].opaque < FULL_OPAQUE) {
                    return chunk.voxels [voxelIndex].packedLightMesh;
                }
                // voxel has contents try to retrieve light information from nearby voxels
                int nearby = voxelIndex + ONE_Y_ROW;
                if (nearby < chunk.voxels.Length && chunk.voxels [nearby].opaque < FULL_OPAQUE) {
                    return chunk.voxels [nearby].packedLightMesh;
                }
                nearby = voxelIndex - ONE_Z_ROW;
                if (nearby >= 0 && chunk.voxels [nearby].opaque < FULL_OPAQUE) {
                    return chunk.voxels [nearby].packedLightMesh;
