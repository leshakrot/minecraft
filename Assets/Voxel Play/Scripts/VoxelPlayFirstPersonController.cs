        ...
        protected virtual void UpdateWithCharacterController ()
        {
            ...
            if (input.focused && input.enabled) {
                ...
                if (input.GetButtonDown (InputButtonNames.Build)) {
                    ...
                    if (env.buildMode) {
                        env.ShowMessage ("<color=green>Включен <color=yellow>режим строительства</color>. Нажмите <color=white>B</color> для отмены.</color>");
                    } else {
                ...
                if (isGrounded && !isCrouched && input.GetButtonDown (InputButtonNames.LeftControl)) {
                    ...
                    if (isCrouched) {
                        env.ShowMessage ("<color=green>Ходьба <color=yellow>сидя</color></color>");
                    } else {
                        env.ShowMessage ("<color=yellow>Обычная <color=green>ходьба</color></color>");
                    }
